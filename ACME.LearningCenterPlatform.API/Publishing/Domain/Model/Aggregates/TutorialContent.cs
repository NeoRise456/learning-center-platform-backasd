...
public partial class Tutorial : IPublishable
{
    ...
    private bool ExistsReadableContentByUrl(string content) => Assets.Any(asset => asset.Type == EAssetType.ReadableContentItem && (string)asset.GetContent() == content);
    
    
    public void AddImage(string imageUrl)
    {
        if (ExistsImageByUrl(imageUrl))
        {
            return;
        }
    ...
    {
        if (ExistsReadableContentByUrl(content))
        {
            return;
        }
